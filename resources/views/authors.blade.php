                        ...
                        <div class="flex min-w-0 gap-x-4 items-center">
                            ...
                            <a href="{{  auth()->user()->hasRole('admin') ?  route('admin.research.index', ['search' => $authorName]) :  route('dashboard.research.index', ['search' => $authorName]) }}">
                                <div class="min-w-0 flex-auto">
                                    <p class="text-sm/6 font-semibold text-gray-900 dark:text-white">{{ $authorName }}</p>
                                    {{-- Removed static email/role/last seen as they are not available per author from the 'authors' string field --}}
                                    {{-- If you had a separate 'authors' table with more details, you would fetch and display them here. --}}
                                </div>
                            </a>
                            @endauth
                            @guest
